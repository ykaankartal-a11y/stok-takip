...
if menu == "🛒 SİPARİŞ AÇ":
    ...
    if st.button("SİPARİŞİ ONAYLA"):
        ...
        st.session_state.data["SIPARISLER"].append({
            ...
            "ADET": adet, "FİYAT": fiyat, "ÜRETİLEN": 0, "DETAY": {}
        })
        verileri_kaydet(st.session_state.data); st.success(f"Sipariş No: {st.session_state.data['SIPARIS_SAYAC']} açıldı!"); st.rerun()

# --- 2. AKTİF SİPARİŞLER ---
elif menu == "📋 AKTİF SİPARİŞLER":
    for i, s in enumerate(st.session_state.data["SIPARISLER"]):
        with st.expander(f"No: {s.get('NO')} | {s.get('MÜŞTERİ')} | {s.get('ÜRÜN')} | Adet: {s.get('ADET')}"):
            ...
            if st.button("🚀 ÜRETİMİ KAYDET", key=f"b_{i}"):
                ...
                # Maliyet hesabı (Hammadde + İşçilik)
                recete = st.session_state.data["RECETELER"].get(s['ÜRÜN'], {})
                for mat, info in recete.items():
                    tutar = miktar * info.get("MİKTAR", 0) * info.get("MALİYET", 0)
                    s["DETAY"][mat] = s.get("DETAY", {}).get(mat, 0) + tutar
...
# --- 3. REÇETE TANIMLA (İşçilik Dahil) ---
elif menu == "⚙️ REÇETE TANIMLA":
    urun = st.text_input("ÜRÜN ADI").upper()
    c1, c2, c3, c4 = st.columns(4)
    h_ad = c1.text_input("Hammadde/İşçilik Adı").upper()
...
# --- 4. MEVCUT REÇETELER (Dinamik Düzenleme) ---
elif menu == "📋 MEVCUT REÇETELER":
    secilen = st.selectbox("ÜRÜN", [""] + list(st.session_state.data["RECETELER"].keys()))
    if secilen:
        st.subheader("➕ Yeni Malzeme/İşçilik Ekle")
        c1, c2, c3, c4 = st.columns(4)
        n_ad = c1.text_input("Ad").upper(); n_mik = c2.number_input("Mik", 0.0)
        n_bir = c3.selectbox("Birim", BIRIM_LISTESI); n_fiy = c4.number_input("Fiyat", 0.0)
        if st.button("Ekle"): st.session_state.data["RECETELER"][secilen][n_ad] = {"MİKTAR": n_mik, "BİRİM": n_bir, "MALİYET": n_fiy}; verileri_kaydet(st.session_state.data); st.rerun()
        
        st.subheader("✏️ Mevcut İçerik")
...
# --- 5. DEPO & 6. ARŞİV (Aynı) ---
    ...
    for s in st.session_state.data.get("ARSIV", []):
        toplam = sum(s.get("DETAY", {}).values())
        with st.expander(f"No: {s.get('NO')} | {s.get('MÜŞTERİ')}"):
            st.metric("Toplam Maliyet", f"{toplam:.2f} ₺")
